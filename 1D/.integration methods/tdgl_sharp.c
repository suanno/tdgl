...
for (loop=0; loop < nloop; loop++){
	...
	for (i=0; i<N; i++){
	integ_coef[i]=1-dt*C[loop]/2; 	//Note that d2coeff = -q^2 (already contains a minus sign)
	...
	/*Crank-Nicolson*/
	for (i=0; i<N; i++){
		udtfr[i]=(ufr[i]*(1+dt*Cprev/2)-dt*NLfr[i])/integ_coef[i];
		udtfi[i]=(ufi[i]*(1+dt*Cprev/2)-dt*NLfi[i])/integ_coef[i];	
	}
